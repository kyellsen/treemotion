...
class Projekt(BaseClass):
    ...
    def load_data_by_version(self, version, session=None):
        ...
        try:
            ...
            return None
        logger.info(f"Prozess zum Laden der Data-Frames für {len(results)} Messreihen aus {self.__str__()} erfolgreich abgeschlossen.")
        return results
    ...
    def copy_version(self, version_new=configuration.data_version_copy_default,
        ...
        try:
            results = self.for_all('messreihen', 'copy_version', version_new, version_source, auto_commit, session)
        except Exception as e:
            logger.error(f"Fehler beim Kopieren aller Data-Objekte für {self.__str__()}, Error: {e}")
            return None
        logger.info(f"Prozess zum Kopieren aller Data-Objekte für {len(results)} Messreihen aus {self.__str__()} erfolgreich abgeschlossen.")
        return results
    ...
    def limit_version_by_time(self, version, start_time: str, end_time: str, auto_commit: bool = False, session=None):
        ...
        try:
            results = self.for_all('messreihen', 'limit_version_by_time', version, start_time, end_time, auto_commit, session)
        except Exception as e:
            logger.error(f"Fehler bei Zeiteinschränkung aller Data-Objekte für {self.__str__()}, Error: {e}")
            return None
        logger.info(f"Prozess zur Zeiteinschränkung aller Data-Objekte für {len(results)} Messreihen aus {self.__str__()} erfolgreich abgeschlossen.")
        return results
