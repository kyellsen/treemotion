...
def sync_data_by_correlation(df_tms: pd.DataFrame, df_wind: pd.DataFrame, max_shift: int,
    ...
    Parameters:
    df_tms: DataFrame containing tilt values.
    df_wind: DataFrame containing wind measurement.
    max_shift: Maximum shift for synchronization, in seconds.
    ...
    try:
        ...
        optimal_shift = find_optimal_shift(df_wind_interpolated, df_tms, 'windgeschwindigkeit', 'neigung',
                                           max_shift, correlation_method, max_window)

        df_tilt_shifted = df_tms.shift(periods=optimal_shift)
